...
import postsPageInitialPaths7980aResource from '../../resources/posts-page-initial-paths-7980a'
import postsPageInitialProps036d2Resource from '../../resources/posts-page-initial-props-036d2'
  ...
  return (
    <>
      <div className="posts11-container">
        <Head>
          <title>Posts1 - Principal Directives Representative</title>
          <meta
            property="og:title"
            content="Posts1 - Principal Directives Representative"
          />
        </Head>
        <DataProvider
...
export async function getStaticPaths() {
  try {
    const response = await postsPageInitialPaths7980aResource({})
    return {
...
export async function getStaticProps(context) {
  try {
    const response = await postsPageInitialProps036d2Resource({
      ...context?.params,
    })
    if (!response?.data?.[0]) {
      return {
